...
CHANGES vs. previous version:
  - is_outdoor / is_indoor populated from the shared categories module.
  - duration_minutes computed via the shared categories module.
  - Photos NO LONGER fetched here — deferred to pipeline after ranking.
  - max_per_category is a real knob (use 5 for dev, 20 for prod).
  - compute_destination_bbox() — derives a rough lat/lon box for the
    destination and drops results outside it. Catches wrong-city matches
    (e.g. "Tokyo Japan" restaurant in Louisiana).
  - fetch_required_attractions() — separate function that fetches user-named
    attractions and marks them with is_mandatory=True + selection_source=
    SOURCE_USER_REQUIRED. These bypass review-count and dedupe checks because
    "the user asked for this" beats every other signal.
"""
...
def parse_attraction(raw: dict, details: dict) -> Attraction:
    ...
    return Attraction(
        ...
        is_outdoor       = is_outdoor(category, subcats),
        is_indoor        = is_indoor(category, subcats),
    )
...
def compute_destination_bbox(
    ...
    """
    Estimate (min_lat, max_lat, min_lon, max_lon) for the destination.

    Uses median lat/lon (robust to outliers) then expands by ~radius_km.
    Returns None if fewer than 3 valid coordinates.
    """
...
def fetch_attractions(
    ...
    """
    Fetch raw Attraction objects, then merge in user-required attractions.

    Required attractions are guaranteed to appear, are marked is_mandatory=True
    and selection_source=SOURCE_USER_REQUIRED, and BYPASS the bbox + review
    filters (the user asked for them by name; we don't second-guess).
    """
    ...
    for cat in categories:
        ...
        if geo_filter and bbox is None and in_this_category:
            ...
            if bbox:
                print(f"    [TA] derived bbox: "
                      f"lat {bbox[0]:.3f}–{bbox[1]:.3f}, "
    ...
    # Required attractions: bypass review filter, ARE bbox-checked (we don't
    # want a Tokyo trip to suddenly include a Louisiana namesake even on user
    # request — likelier they mistyped). But we mark them mandatory.
    required = fetch_required_attractions(ta, prefs, bbox=bbox, sleep_s=sleep_s)

    # Merge — required wins on duplicates (we set is_mandatory)
    existing_ids = {a.location_id for a in attractions}
    for req in required:
        if req.location_id in existing_ids:
            # bump the existing one up to mandatory status
...
def fetch_required_attractions(
    ...
    Returns attractions with is_mandatory=True. Skips the min-reviews check
    (user asked for it by name — trust them) but still applies bbox filtering
    when a bbox is provided.
    """
    out: list[Attraction] = []
    for req in prefs.required_attractions:
        ...
        if not results:
            print(f"    no results for required '{req}' — skipping")
            continue
        # take the top result (best name match)
        for r in results[:3]:
            try:
                details = ta.get_location_details(r["location_id"])
                a = parse_attraction(r, details)
                ...
                if bbox is not None and not _inside_bbox(a, bbox):
                    print(f"    skip required '{a.name}': outside destination bbox")
                    continue
                ...
                out.append(a)
                time.sleep(sleep_s)
                break  # one match per required name
            except Exception as exc:
...
def attach_photos(ta: TripAdvisorClient,
    ...
    """Fetch and attach photo URLs in-place. Call AFTER ranking, for top-k only."""
    for a in attractions:
        if a.photo_url:
            continue
        try:
